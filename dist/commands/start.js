...
exports.default = registerStartCommand;
const grammy_1 = require("grammy");
function registerStartCommand(bot) {
    bot.command("start", async (ctx) => {
        // Inline button (top image buttons)
        const inline = new grammy_1.InlineKeyboard().text("ANNOUNCEMENTS", "action:announcements");
        // Reply keyboard buttons (bottom big buttons)
        const replyKb = new grammy_1.Keyboard()
...
<b>Lock TWT to amplify your benefits and unlock exclusive perks.</b>
    `;
        await ctx.replyWithPhoto("https://i.postimg.cc/WbZwSZTn/trust-wallet-start.jpg", {
            caption: messageText,
            parse_mode: "HTML",
            reply_markup: inline,
        });
